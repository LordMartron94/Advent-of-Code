...
func (g *GetInputDataPipe) Process(input common.PipelineContext[task_rules.LexingTokenType]) common.PipelineContext[task_rules.LexingTokenType] {
	...
	lexingRules := []rules.LexingRuleInterface[task_rules.LexingTokenType]{
		lexerRuleFactory.GetDigitTokenRuleLex(),
		lexerRuleFactory.GetPipeTokenRuleLex(),
		lexerRuleFactory.GetCommaTokenRuleLex(),
		lexerRuleFactory.GetWhitespaceTokenRuleLex(),
		lexerRuleFactory.GetInvalidTokenRuleLex(),
	}

	parsingRules := []rules2.ParsingRuleInterface[task_rules.LexingTokenType]{
		parsingRuleFactory.GetPairOfNumbersParserRule(),
		parsingRuleFactory.GetUpdateCommandParserRule(),
		parsingRuleFactory.GetWhitespaceTokenParserRule(),
	...
	//	if token.Type == task_rules.WhitespaceToken {
	//		fmt.Println("Encountered Whitespace Token")
	//		continue
	...
	//tree.Print(2, []task_rules.LexingTokenType{task_rules.WhitespaceToken})
	//
	//os.Exit(0)
